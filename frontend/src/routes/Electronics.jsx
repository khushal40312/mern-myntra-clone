...
export default function Electronics() {
    const itemse = useSelector(store => store.items)
    ...
    <img id="mobileImage" src="images/gadgetbanner.avif" alt="Mobile Image" />
              <div className="items-container">
                  {itemse.filter(item => item.category === 'Electronic').map(items => <ElectronicsItems key={items.id} items={items} allItems={itemse}  />)}
